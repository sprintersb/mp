#ifndef EXPR_PARSE_DOUBLE_H
#define EXPR_PARSE_DOUBLE_H

namespace
{
    template<typename F> struct ExpressionParserFarg { using T = F; };
};
...
{
public:
    using Farg = typename ExpressionParserFarg<F>::T;
    ...
    public:
        ...
        void set (const char *var, Farg value) const;
    ...
    /* Parse string TEXT and return an abstract representation that can be
       ...
       Known constants
            ...
            EPS             Machine Epsilon
            PREC            Current precision of the mantissae (in bits)
       ...
       Variable number of Arguments
            min (x, ...)            Minimum
            max (x, ...)            Maximum
            hypot (x, ...)          Diagonal of an n-dimensional box
    ...
    */
    ...
    static F eval (const char *text, Farg x, int flags = 0);
...
extern template class ExpressionParser<double>;

#endif // EXPR_PARSE_DOUBLE_H
